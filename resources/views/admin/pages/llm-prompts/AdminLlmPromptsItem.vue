...
import VersionIcon from '@/components/icons/VersionIcon.vue'
...
const makeCurrentVersion = async (versionId) => {
    loading.value = true;
    updated.value = false;
    try {
        const response = await axios.post('/api/admin/llm-prompts/version/'+versionId+'/current',{
            headers: {
        ...
        if (axios.isAxiosError(error)) {
            console.error('Error occurred 1:', error.response?.data);
        } else {
            console.error('Error occurred 2:', error);
        }
    }
    loading.value = false;
};
...
<template>
    <tr class="bg-white border-b">
        <td class="px-6 py-4 text-center">
            <p class="font-extrabold text-md text-black">{{ prompt.name }}</p>
        </td>

        <td class="px-6 py-4 text-center">
            {{ prompt.current_version?.name }}
        </td>

        <td class="px-6 py-4 text-center">
            <div class="flex text-center">
                {{ prompt.versions?.length }}
                <a href="" @click.prevent="toggleShowVersions">
                    <VersionIcon width="20px" height="20px" fill="#C1BCB8"/>
                </a>
            </div>
        </td>

        <td class="px-6 py-4 text-center">
            <p class="text-black">{{ prompt.updated_at }}</p>
            <p>{{ prompt.updated_by?.name }}</p>
        </td>
    ...
    <TransitionGroup name="fade">
        <tr v-if="showVersions" class="bg-gray-100">
            <td colspan="5" class="px-3 py-2">
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                        <tr v-for="(version, index) in prompt.versions" :key="index" class="border-b">
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                                {{ version.name }}
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                                <p>{{ version.updated_by }}</p>
