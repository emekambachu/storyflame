    ...
    >
        <aside class="flex flex-col w-full">
            ...
            <div class="flex items-center mb-14">
                <progress-bar
                    :percent="Math.min(Object.keys(data).length, questionsCount) / questionsCount * 100"
                    class="w-full"
                />
            </div>
        </aside>
        ...
        <div class="flex justify-center mt-auto w-full">
            ...
            >
                Switch to
                <span class="text-red-600 font-bold">{{ isSpeakingMode ? 'typing' : 'speaking' }}</span>
                mode
            </button>
...
const data = ref(JSON.parse(localStorage.getItem('onboarding.user') || 'null') || {})
...
const lastQuestion = computed(() => {
    return messages.value.filter((message) => message.type === 'question' || message.type === 'finish').at(-1)
})
...
function extractData(answer: string) {
    ...
    axios.post<SuccessResponse<any>>('/api/v1/onboarding', {
        ...
        localStorage.setItem('onboarding.messages', JSON.stringify(messages.value))
