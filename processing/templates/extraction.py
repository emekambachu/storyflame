...
category_extraction_template = (
    ...
    "CategoryCriteria.  The criteria will include positive notes about what may constitute the category having been "
    "discussed, as well as negative notes about what does not constitute the category being discussed.  Please rate "
    "the confidence level that the Category was discussed in the CurrentContext.writer_response between 0.0-0.1 = "
    "no_confidence, 0.2-0.5 = low_confidence, 0.6-0.9 = high_confidence, and 0.9-1.0 for absolute confidence.  Then "
    "add each potential Category Element mentioned in an array for that Category with information about the element "
    "and a confidence rating for each. Only include elements with high or absolute confidence."
    ...
    "Sam's demographics, physical appearance, motivation, and friends.\",\"confidence\": 0.8}}, {{\"name\": \"Sam's "
    "Sister\",\"usage\": \"While not named, Sam's sister is mentioned as a supporting character who helps Sam.\","
    "\"confidence\": 0.8}}]}}"
    ...
    "<json_response></json_response> XML tags.  The JSON should be in {{categories:[{{type:category_type, confidence: "
    "confidence_level, elements:[{{\"name\":name_if_found, \"usage\": summary_sentences_of_how_it_was_mentioned, "
    "\"confidence\": confidence_rating}}]}}]}} format.  Only include the <json_response> content, no other context is "
    "needed."
)
